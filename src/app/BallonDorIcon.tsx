...
export function BallonDorIcon({
  className,
  // Deux instances sur la même page (ex: en-tête + filigrane géant, voir BallonDorBackground) ne
  // peuvent pas partager le même id de <linearGradient> — un id HTML doit rester unique dans le
  // document, sinon la seconde url(#bdor-gold) référence arbitrairement l'une des deux défs.
  gradientId = "bdor-gold",
}: {
  className?: string;
  gradientId?: string;
}) {
  return (
    <svg viewBox="0 0 64 64" fill="none" className={className} aria-hidden>
      ...
      {/* Ballon */}
      <circle cx="32" cy="32" r="17" fill={`url(#${gradientId})`} />
